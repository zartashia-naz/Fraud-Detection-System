...
# from pydantic import BaseModel  # ✅ Added for OTP body

# # 🔽 ADD IMPORTS (do NOT remove existing ones)
# from app.core.security import (
#     hash_password,
#     verify_password,
#     create_access_token,
#     create_temp_token,      # ✅ TEMP TOKEN
#     get_current_temp_user   # ✅ TEMP TOKEN DEPENDENCY
# )
...
#     # ❌ Wrong password - CREATE LOG AND CHECK RISK
...
#         # 🚨 BLOCK HIGH-RISK AFTER FAILED LOGIN
#         if context["risk_score"] is not None and context["risk_score"] >= HIGH_RISK_THRESHOLD:
...
#         # Not high risk, return normal error
#         raise HTTPException(status_code=400, detail="Invalid email or password")

#     # ✅ Password correct - Create successful login log
#     login_log_id, context = await _create_login_log(
#         db=db,
#         email=user["email"],
...
#     # 🚨 BLOCK HIGH-RISK ACCOUNTS (SUCCESSFUL LOGIN)
#     if context["risk_score"] is not None and context["risk_score"] >= HIGH_RISK_THRESHOLD:
#         blocked_until = datetime.utcnow() + timedelta(hours=1)
#         await db.users.update_one(
#             {"_id": user["_id"]},
...
#     # 🔐 OTP REQUIRED → SEND OTP & TEMP TOKEN
#     if otp_required:
...
#     current_user=Depends(get_current_temp_user),  # ✅ TEMP TOKEN
# ):
#     try:
#         await verify_otp(
#             db=db,
...
#     # ✅ ISSUE REAL ACCESS TOKEN AFTER OTP
#     token = create_access_token({
#         "id": current_user["id"],
