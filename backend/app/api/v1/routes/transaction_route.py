...
# # --------------------------------------------------
# # CREATE TRANSACTION (OTP NOT sent automatically)
# # --------------------------------------------------
# @router.post("")
...
#         metadata={"transaction_id": str(payload.transaction_id)},
#     )
...
#     # Fetch from anomalies (blocked)
#     blocked_list = []
#     blocked_query = {**base_query, **date_filter, **amount_filter, **cat_filter}
#     if not status or status == "blocked":
#         blocked_list = await db.anomalies.find(blocked_query).to_list(1000)

#     all_txns = completed_resolved + flagged_list + blocked_list

#     # Sort the combined list
#     if sort_by == "transaction_date":
#         all_txns.sort(key=lambda x: x["transaction_date"], reverse=desc)
#     elif sort_by == "amount":
#         all_txns.sort(key=lambda x: x["amount"], reverse=desc)
#     elif sort_by == "status":
#         all_txns.sort(key=lambda x: x["status"], reverse=desc)
#     # else: default to transaction_date, already sorted or not
...
async def create_transaction(
    ...
    # High risk
    txn_dict["status"] = "blocked"
    result = await db.anomaly_logs.insert_one(txn_dict)
    txn_id = result.inserted_id
    ...
    anomaly_log = await db.anomaly_logs.find_one({"_id": txn_id})
    reason_summary = anomaly_log.get("reason_summary", "suspicious activity detected")
    ...
    email_body = f"Your account has been blocked for 3 hours due to {reason_summary}. Please contact support if you believe this is an error."
...
async def get_transactions(
    ...
    # Fetch from anomaly_logs (blocked)
    blocked_list = []
    blocked_query = {**base_query, **date_filter, **amount_filter, **cat_filter}
    if not status or status == "blocked":
        blocked_list = await db.anomaly_logs.find(blocked_query).to_list(1000)
